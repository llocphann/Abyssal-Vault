...
if (root) {
  ...
  const categories = (layout = layoutState()) => {
    const customIds = [...layout.tabOrder, ...Object.keys(layout.tabs)].filter((id, index, all) => id && all.indexOf(id) === index && !baseCategoryIds.has(id));
    const custom = customIds.map(id => {
      const data = copyObject(layout.tabs[id]);
      return {
        id,
        label: String(data.label || id),
        eyebrow: String(data.eyebrow || "Custom"),
        description: String(data.description || "Custom settings tab."),
        custom: true,
      };
    });
    return [...BASE_CATEGORIES.map(item => ({ ...item, custom: false })), ...custom];
  };

  const sections = (layout = layoutState()) => {
    const validTabs = new Set(categories(layout).map(item => item.id));
    const base = BASE_SECTIONS.map(item => ({
      ...item,
      category: validTabs.has(layout.headingTabs[item.id]) ? layout.headingTabs[item.id] : item.category,
      custom: false,
    }));
    const customIds = Object.keys(layout.headings).filter(id => !baseSectionIds.has(id));
    const custom = customIds.map(id => {
      const data = copyObject(layout.headings[id]);
      const category = validTabs.has(layout.headingTabs[id]) ? layout.headingTabs[id] : "advanced";
      return {
        id,
        category,
        label: String(data.label || id),
  ...
  const metaFor = (key, value) => {
    ...
    return { ...base, category: validTabs.has(base.category) ? base.category : "advanced", section: null };
  };
  ...
  const updateCustomHeading = async (headingId, label, note) => {
    const layout = layoutState();
    const section = sectionFor(headingId, layout);
    if (!section?.custom) throw Error("Only custom headings can be edited.");
    layout.headings[headingId] = { ...copyObject(layout.headings[headingId]), label, note };
    await saveLayout(layout);
  };

  const deleteCustomHeading = async headingId => {
    const layout = layoutState();
    const section = sectionFor(headingId, layout);
    if (!section?.custom) throw Error("Only custom headings can be deleted.");
  ...
  const updateCustomTab = async (tabId, label, description) => {
    const layout = layoutState();
    const tab = categories(layout).find(item => item.id === tabId);
    if (!tab?.custom) throw Error("Only custom tabs can be edited.");
    layout.tabs[tabId] = {
      ...copyObject(layout.tabs[tabId]),
      label,
      description: description || "Custom settings tab.",
    };
    await saveLayout(layout);
  };
  ...
  const deleteCustomTab = async tabId => {
    const layout = layoutState();
    const tab = categories(layout).find(item => item.id === tabId);
    if (!tab?.custom) throw Error("Only custom tabs can be deleted.");
    const fallbackTab = "advanced";
    const allEntries = entries();
    ...
    layout.tabOrder = layout.tabOrder.filter(id => id !== tabId);
    await saveLayout(layout);
  };
  ...
  function openHeadingEditor(block, section) {
    if (!section.custom || searchTerm) return;
    finishDrag();
    ...
    remove.onclick = async () => {
      ...
      try {
        await deleteCustomHeading(section.id);
        setStatus("Saved");
        render();
      } catch (deleteError) {
    ...
    form.onsubmit = async event => {
      ...
      try {
        await updateCustomHeading(section.id, label, description.value.trim());
        setStatus("Saved");
        render();
      } catch (saveError) {
        remove.disabled = false;
  ...
  async function openTabForm(category = null) {
    ...
    const editing = category?.custom ? category : null;
    searchTerm = "";
    const search = q('[data-role="settings-search"]');
    if (search) search.value = "";
    render();
    ...
    const { form, name, actions, cancel, save, error } = layoutForm("tab", async (label, description) => {
      if (editing) {
        setStatus(`Saving ${label}…`, "saving");
        await updateCustomTab(editing.id, label, description);
        active = editing.id;
      } else {
        const layout = layoutState();
        const used = new Set(categories(layout).map(item => item.id));
        const id = uniqueId("tab", label, used);
        layout.tabs[id] = { label, eyebrow: "Custom", description: description || "Custom settings tab." };
        layout.tabOrder.push(id);
    ...
    if (editing) {
      ...
      remove.onclick = async () => {
        ...
        try {
          await deleteCustomTab(editing.id);
          active = "advanced";
          persistState();
          setStatus("Saved");
          render();
  ...
  function openHeadingForm() {
    ...
    const { form, name } = layoutForm("heading", async (label, description) => {
      const layout = layoutState();
      const used = new Set(sections(layout).map(item => item.id));
  ...
  function renderCategory(allEntries) {
    ...
    if (editTab) editTab.hidden = !category?.custom;
  ...
  function openCustomForm() {
    const tabs = categories();
    const targetTab = tabs.some(item => item.id === active) ? active : "advanced";
    searchTerm = "";
    persistState();
    const search = q('[data-role="settings-search"]');
    if (search) search.value = "";
  ...
  q('[data-role="settings-edit-tab"]').onclick = () => {
    ...
    if (category?.custom) openTabForm(category);
  };
